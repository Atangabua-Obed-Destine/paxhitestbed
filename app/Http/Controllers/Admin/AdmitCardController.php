...
class AdmitCardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(isset($request->faculty) || isset($request->program) || isset($request->session) || isset($request->semester) || isset($request->section) || isset($request->student_id)){
            ...
            if(!empty($request->student_id)){
                $students->where(function($query) use ($student_id) {
                    $query->where('student_id', 'LIKE', '%'.$student_id.'%')
                          ->orWhereHas('studentEnrolls', function($q) use ($student_id) {
                              $q->where('matricule', 'LIKE', '%'.$student_id.'%');
                          });
                });
            }
            $rows = $students->orderBy('matricule', 'asc')->get();
    ...
    public function print($id)
    {
        ...
        // View
        $data['rows'] = Student::where('id', $id)->orderBy('matricule', 'asc')->get();
    ...
    public function multiPrint(Request $request)
    {
        ...
        // View
        $data['rows'] = Student::whereIn('id', $students)->orderBy('matricule', 'asc')->get();
        $data['print'] = PrintSetting::where('slug', 'admit-card')->firstOrFail();

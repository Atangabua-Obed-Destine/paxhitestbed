              ...
              <form class="needs-validation" novalidate action="{{ route($route.'.pay', $payroll_data->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            <label for="bank_account_id" class="form-label">{{ __('field_bank_account') }} <span>*</span></label>
                            <select class="form-control" name="bank_account_id" id="bank_account_id" required>
                                <option value="">{{ __('select') }}</option>
                                @php
                                    $accounts = $row->bankAccounts->where('status', 1);
                                    $defaultAccount = $accounts->where('is_default', 1)->first();
                                @endphp
                                ...
                                @empty
                                    <option value="" disabled>{{ __('no_bank_accounts') }}</option>
                                @endforelse
                            </select>

                            <div class="invalid-feedback">
                              {{ __('required_field') }} {{ __('field_bank_account') }}
                            </div>
                        </div>

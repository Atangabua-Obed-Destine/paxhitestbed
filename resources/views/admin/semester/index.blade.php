                        ...
                        <div class="card-block">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="semester_type" id="semester_type" required>
                                    <option value="">{{ __('select') }}</option>
                                    <option value="1" @if( old('semester_type') == 1 ) selected @endif>{{ __('First Semester') }}</option>
                                    <option value="2" @if( old('semester_type') == 2 ) selected @endif>{{ __('Second Semester') }}</option>
                                </select>
                            ...
                            <div class="form-group">
                                <div class="checkbox d-inline">
                                    <input type="checkbox" name="is_resit" id="is_resit_create" value="1" {{ old('is_resit') ? 'checked' : '' }} onchange="toggleParentSemester('create')">
                                    <label for="is_resit_create" class="cr">{{ __('Resit Semester') }}</label>
                                </div>
                                <small class="form-text text-muted">{{ __('Mark this semester as dedicated to resit activities.') }}</small>
                            </div>

                            <div class="form-group" id="parent_semester_create" style="display: {{ old('is_resit') ? 'block' : 'none' }};">
                                <label for="parent_semester_id_create" class="form-label">{{ __('Parent Semester') }}</label>
                                <select class="form-control" name="parent_semester_id" id="parent_semester_id_create">
                                    <option value="">{{ __('select') }}</option>
                                    @foreach($regular_semesters as $regular_semester)
                                    <option value="{{ $regular_semester->id }}" @if(old('parent_semester_id') == $regular_semester->id) selected @endif>
                            ...
                            <table id="basic-table" class="display table nowrap table-striped table-hover" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ __('field_year') }}</th>
                                        <th>{{ __('Semester Type') }}</th>
                                        <th>{{ __('Resit Semester') }}</th>
                                        <th>{{ __('Parent Semester') }}</th>
                                  ...
                                  @foreach( $rows as $key => $row )
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <span class="badge badge-success">{{ __('First Semester') }}</span>
                                            @elseif($row->semester_type == 2)
                                            <span class="badge badge-warning">{{ __('Second Semester') }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if($row->is_resit)
                                        ...
                                        <td>
                                            @if($row->is_resit && $row->parentSemester)
                                            <span class="badge badge-secondary">{{ $row->parentSemester->title }}</span>
                                            @else
...
<script type="text/javascript">
    ...
    // Toggle parent semester dropdown based on is_resit checkbox
    function toggleParentSemester(id) {
        var checkbox = $('#is_resit_' + id);
        var parentSemesterDiv = $('#parent_semester_' + id);
        
        if (checkbox.is(':checked')) {
            parentSemesterDiv.show();
        } else {
            parentSemesterDiv.hide();
            $('#parent_semester_id_' + id).val('');
        }
    }
</script>
@endsection
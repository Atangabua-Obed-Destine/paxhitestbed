                    ...
                    @method('PUT')
                        ...
                        <content class="form-step">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <select class="form-control select2-multiple" name="roles[]" id="role" required multiple>
                        ...
                        <h3>{{ __('tab_accounts') }}</h3>
                        <content class="form-step">
                            <!-- Form Start--->
                            <fieldset class="row scheduler-border">
                                <div class="col-md-12">
                                    <div class="alert alert-info">
                                        <i class="fas fa-info-circle"></i> Manage bank accounts for this staff member. These will be available for selection during payroll payment.
                                    </div>
                                    
                                    <div id="bank-accounts-container">
                                        @php
                                            $existingAccounts = $row->bankAccounts;
                                            $accountCount = $existingAccounts->count();
                                        @endphp
                                        
                                        @if($accountCount > 0)
                                            @foreach($existingAccounts as $index => $account)
                                            <div class="card mb-3 account-card" data-account-id="{{ $account->id }}">
                                                <div class="card-header">
                                                    <strong>Account {{ $index + 1 }}</strong>
                                                    <button type="button" class="btn btn-sm btn-danger float-right remove-account" {{ $accountCount == 1 ? 'style=display:none;' : '' }}>
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <div class="row">
                                                        <div class="form-group col-md-4">
                                                            <label>{{ __('field_bank_name') }} <span>*</span></label>
                                                            <input type="text" class="form-control" name="accounts[{{ $index }}][bank_name]" value="{{ $account->bank_name }}" required>
                                                        </div>
                                                        <div class="form-group col-md-4">
                                                            <label>{{ __('field_account_name') }} <span>*</span></label>
                                                            <input type="text" class="form-control" name="accounts[{{ $index }}][account_name]" value="{{ $account->account_name }}" required>
                                                        </div>
                                                        <div class="form-group col-md-4">
                                                            <label>{{ __('field_account_number') }} <span>*</span></label>
                                                            <input type="text" class="form-control" name="accounts[{{ $index }}][account_number]" value="{{ $account->account_number }}" required>
                                                        </div>
                                                        <div class="form-group col-md-4">
                                                            <label>{{ __('field_bank_brach') }}</label>
                                                            <input type="text" class="form-control" name="accounts[{{ $index }}][branch]" value="{{ $account->branch }}">
                                                        </div>
                                                        <div class="form-group col-md-4">
                                                            <label>{{ __('field_ifsc_code') }}</label>
                                                            <input type="text" class="form-control" name="accounts[{{ $index }}][ifsc_code]" value="{{ $account->ifsc_code }}">
                                                        </div>
                                                        <div class="form-group col-md-4">
                                                            <label class="d-block">{{ __('field_default_account') }}</label>
                                                            <div class="checkbox d-inline">
                                                                <input type="radio" name="default_account" value="{{ $index }}" id="default_{{ $index }}" {{ $account->is_default ? 'checked' : '' }}>
                                                                <label for="default_{{ $index }}" class="cr">Set as Default</label>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        @else
                                            <!-- Account 1 (Default) -->
                                            <div class="card mb-3 account-card">
                                                <div class="card-header">
                                                    <strong>Account 1</strong>
                                                    <button type="button" class="btn btn-sm btn-danger float-right remove-account" style="display: none;">
                                                        <i class="fas fa-times"></i> Remove
                                                    </button>
                                                </div>
                                                <div class="card-body">
                                                    <div class="row">
                                                        <div class="form-group col-md-4">
                                                            <label>{{ __('field_bank_name') }} <span>*</span></label>
                                                            <input type="text" class="form-control" name="accounts[0][bank_name]" required>
                                                        </div>
                                                        <div class="form-group col-md-4">
                                                            <label>{{ __('field_account_name') }} <span>*</span></label>
                                                            <input type="text" class="form-control" name="accounts[0][account_name]" required>
                                                        </div>
                                                        <div class="form-group col-md-4">
                                                            <label>{{ __('field_account_number') }} <span>*</span></label>
                                                            <input type="text" class="form-control" name="accounts[0][account_number]" required>
                                                        </div>
                                                        <div class="form-group col-md-4">
                                                            <label>{{ __('field_bank_brach') }}</label>
                                                            <input type="text" class="form-control" name="accounts[0][branch]">
                                                        </div>
                                                        <div class="form-group col-md-4">
                                                            <label>{{ __('field_ifsc_code') }}</label>
                                                            <input type="text" class="form-control" name="accounts[0][ifsc_code]">
                                                        </div>
                                                        <div class="form-group col-md-4">
                                                            <label class="d-block">{{ __('field_default_account') }}</label>
                                                            <div class="checkbox d-inline">
                                                                <input type="radio" name="default_account" value="0" id="default_0" checked>
                                                                <label for="default_0" class="cr">Set as Default</label>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    <button type="button" class="btn btn-success" id="add-account">
...
@section('page_js')
    ...
    (function ($) {
        ...
        var accountIndex = {{ $accountCount ?? 1 }}; // Continue from existing accounts
        ...
        $('#add-account').click(function() {
            var newAccount = `
                <div class="card mb-3 account-card">
                    <div class="card-header">
                        <strong>Account ${accountIndex + 1}</strong>
                        <button type="button" class="btn btn-sm btn-danger float-right remove-account">
                            <i class="fas fa-times"></i> Remove
                        </button>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label>{{ __('field_bank_name') }} <span>*</span></label>
                                <input type="text" class="form-control" name="accounts[${accountIndex}][bank_name]" required>
                            </div>
                            <div class="form-group col-md-4">
                                <label>{{ __('field_account_name') }} <span>*</span></label>
                                <input type="text" class="form-control" name="accounts[${accountIndex}][account_name]" required>
                            </div>
                            <div class="form-group col-md-4">
                                <label>{{ __('field_account_number') }} <span>*</span></label>
                                <input type="text" class="form-control" name="accounts[${accountIndex}][account_number]" required>
                            </div>
                            <div class="form-group col-md-4">
                                <label>{{ __('field_bank_brach') }}</label>
                                <input type="text" class="form-control" name="accounts[${accountIndex}][branch]">
                            </div>
                            <div class="form-group col-md-4">
                                <label>{{ __('field_ifsc_code') }}</label>
                                <input type="text" class="form-control" name="accounts[${accountIndex}][ifsc_code]">
                            </div>
                            <div class="form-group col-md-4">
                                <label class="d-block">{{ __('field_default_account') }}</label>
                                <div class="checkbox d-inline">
                                    <input type="radio" name="default_account" value="${accountIndex}" id="default_${accountIndex}">
                                    <label for="default_${accountIndex}" class="cr">Set as Default</label>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            $('#bank-accounts-container').append(newAccount);
            accountIndex++;
            
            // Show remove button on all accounts if more than one exists
            if ($('.account-card').length > 1) {
                $('.remove-account').show();
            }
        });
        ...
        $(document).on('click', '.remove-account', function() {
            if ($('.account-card').length > 1) {
                var accountCard = $(this).closest('.account-card');
                var accountId = accountCard.data('account-id');
                
                // If editing an existing account, mark it for deletion
                if (accountId) {
                    ...
                    $('<input>').attr({
                        ...
                        value: accountId
                    }).appendTo('#bank-accounts-container');
                }
                
                accountCard.remove();
                
                // Hide remove button if only one account remains
                if ($('.account-card').length === 1) {
                    $('.remove-account').hide();
                }
                
                // Update account numbers
                $('.account-card').each(function(index) {
                    $(this).find('.card-header strong').text('Account ' + (index + 1));
                });
            } else {

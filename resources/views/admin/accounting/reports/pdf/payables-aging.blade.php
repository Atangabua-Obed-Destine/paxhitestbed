...
@section('report_title', __('Payables Aging Report'))
...
@section('content')
    ...
    <!-- Details Table -->
    <div class="section-title">{{ __('Aging Details by Supplier') }}</div>
    <table>
        <thead>
            <tr>
                <th>{{ __('Supplier') }}</th>
        ...
        <tbody>
            ...
            @empty
            <tr>
                <td colspan="7" class="text-center">{{ __('No payables data found') }}</td>
            </tr>
            @endforelse
        </tbody>
        @if(isset($data['summary']))

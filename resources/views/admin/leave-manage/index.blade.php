                                ...
                                <tbody>
                                  @foreach( $rows as $key => $row )
                                    <tr>
                                        ...
                                        <td>
                                            {{ $row->leaveType->title ?? '' }}
                                            @if($row->leaveType && $row->leaveType->limit > 0)
                                                @php
                                                    $balYear = (int) date('Y', strtotime($row->from_date));
                                                    $balUsed = \App\Models\Leave::usedDaysForType($row->user_id, $row->leaveType->id, $balYear, [1]);
                                                @endphp
                                                <br>
                                                <span class="badge badge-pill {{ $balUsed >= $row->leaveType->limit ? 'badge-danger' : 'badge-light' }}"
                                                      title="{{ __('Approved days used / annual limit') }} ({{ $balYear }})">
                                                    {{ $balUsed }}/{{ $row->leaveType->limit }}
                                                </span>
                                            @endif
                                        </td>
                                        <td>
